...
ss_list = [al_cl_ss, al_f_ss, al_br_ss, al_i_ss, epo_ss,
           op_o_ss, op_s_ss, carboxyl_ester_ss, lactone_ss, carbonate_ss,
           cyclic_carbonate_ss, anhydride_ss, cyclic_anhydride_ss, amide_ss, lactam_ss, carbamate_ss, thiocarbamate_ss, 
           urea_ss, sulfonylurea_ss, nitrile_ss, NS_ss, imide_ss,
           acid_cl_ss, acid_br_ss, acid_f_ss, acid_i_ss, PDK]

ss_label = ['aliphatic chloride', 'aliphatic fluoride', 'aliphatic bromide', 'aliphatic iodide',
            'epoxide', 'organophosphorus ester with oxygen', 'organophosphorus ester with sulphur', 
            'carboxylic acid ester', 'lactone', 'carbonate', 'cyclic carbonate', 'anhydride', 'cyclic anhydride',
            'amide', 'lactam', 'carbamate', 'thiocarbamate', 'urea', 'sulfonylurea', 'nitrile', 'Nitrogen-Sulphur', 
            'imide','acid chloride', 'acid bromide', 'acid fluoride', 'acid iodide', 'PDK']
...
def get_fg_dict(rdkit_mol):
    ...
    for fg_s in zip(fg_i_t,fg_n_t):
        ...
        for x in zip(fg_i_t, fg_n_t):  
            if ( 
                (len(set(x[0])) > len(set(fg_s[0]))) and 
                            set(x[0]).issuperset(set(fg_s[0]))
            ):
