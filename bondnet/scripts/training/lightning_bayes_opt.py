...
class TrainingObject:
    def __init__(self, config, log_save_dir, project_name, dataset_loc):
        ...
        self.dataset = ReactionNetworkDatasetPrecomputed(
            ...
            classif_categories=self.config["parameters"]["classif_categories"][
                "values"
            ][0],
            ...
            filter_sparse_rxns=self.config["parameters"]["filter_sparse_rxns"][
                "values"
            ][0],
        ...
        if bool(self.config["parameters"]["transfer"]["values"][0]):
            self.dataset_transfer = ReactionNetworkDatasetPrecomputed(
                ...
                filter_species=self.config["parameters"]["filter_species"]["values"][0],
                filter_outliers=self.config["parameters"]["filter_outliers"]["values"][
                    0
                ],
                filter_sparse_rxns=self.config["parameters"]["filter_sparse_rxns"][
                    "values"
    ...
    def make_model(self, config):
        ...
        model = load_model_lightning(
            config, device=self.device, load_dir=self.log_save_dir
        )
        return model
        ...
        with wandb.init(project=self.wandb_name) as run:
            ...
            if config["transfer"]:
                ...
                logger_tb_transfer = TensorBoardLogger(
                    log_save_dir, name=self.log_save_dir + "_transfer"
                )
                logger_wb_transfer = WandbLogger(
                    project=wandb_project_name, name=self.log_save_dir + "_transfer"
                )
                ...
                trainer_transfer.fit(
                    model, train_loader_transfer, self.val_loader_transfer
                )
            ...
            trainer.fit(model, train_loader, self.val_loader)

            trainer.test(model, self.test_loader)
