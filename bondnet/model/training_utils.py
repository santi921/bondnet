...
def train_classifier(
    ...
    if weight == None:
        weight = torch.tensor([1.0 for i in range(categories)])
    else:
    ...
    for it, (batched_graph, label) in enumerate(data_loader):
        ...
        if augment and not empty_aug:
            ...
            loss = loss_fn(
                torch.concat([pred, pred_aug]),
                torch.flatten(torch.concat([target_filtered, target_filtered_aug])),
            )
...
def train(
    model, nodes, data_loader, optimizer, loss_fn="mse", device=None, augment=False
):
    """
    basic loop for training a classifier. Gets loss and accuracy
