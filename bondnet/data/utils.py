...
def ring_features_for_bonds_full(bonds, no_metal_binary, cycles, allowed_ring_size):
    ...
    for i, bond in enumerate(bonds): 
        if(no_metal_binary[i] == 0):
            ret_dict[tuple(bond)] = (0, ring_features_from_bond(bond, cycles, allowed_ring_size)[0],
...
def organize_list(cycle_list):
    ...
    for cycle in cycle_list: 
        ...
        if(plus_1>cycle_len-1):plus_1-=cycle_len
        ...
        for ind, node in enumerate(cycle): 
            ...
            if(ind_next>cycle_len-1):ind_next-=cycle_len
    ...
    for i in range(num_atoms):
        for j in range(num_atoms):
            ...
            if not bond is None: # checks if rdkit has detected the bond
                ...
                ft += one_hot_encoding(bond.GetBondType(), allowed_bond_type) # bond type 
